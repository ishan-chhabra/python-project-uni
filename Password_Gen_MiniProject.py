...
class GUI():
    ...
    def __init__(self, master):
        ...
        self.label = Label(text="Password Generator",
                           fg='darkmagenta', font='times 35 underline bold')
        ...
        self.user = Label(text="Enter the name of the user:",
        ...
        self.length = Label(text="Enter the length:",
                            font='times 25 bold italic')
        ...
        self.generated_password = Label(
            text="Generated Password:", font='times 25 bold italic')
        self.generated_password.grid(row=9, column=0)

        self.generated_password_textfield = Entry(
            font='times 20', bd=6, relief='groove', fg='darkgreen')
        ...
        self.generate = Button(text="Generate Password", bd=4, relief='solid', padx=1, pady=1,
                               font='times 14 bold', fg='maroon', bg='limegreen', command=self.generate_pass)
        self.generate.grid(row=12, column=0, columnspan=3)

        self.reset = Button(text="Reset", bd=4, relief='solid', padx=1, pady=1,
                            font='times 25 bold', fg='darkblue', bg='red', command=self.reset_fields)
    ...
    def generate_pass(self):
        ...
        if leng.isalpha() == True:
            messagebox.showwarning(
                "Warning", "Length must be an interger >= 6!")
            self.length_textfield.delete(0, 25)
            return

        if leng == '':
            self.blank_label2.configure(
                text="Length field cannot be empty!", font='times 20 bold', fg='blue')
        ...
        if length < 6:
            self.blank_label1.configure(
                text="Password must be atleast 6 characters long!!!", font='times 20 italic', fg='green')
            self.blank_label2.configure(text="")
            return
        else:
            self.blank_label1.configure(text="")
        ...
        n = length-u-l-c
        password = random.sample(upper, u)+random.sample(lower, l) + \
            random.sample(chars, c)+random.sample(numbers, n)
